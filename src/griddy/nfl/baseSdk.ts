...
import { removeSuffix, templateUrl } from "../core";
...
import { GriddyNFLDefaultError, NoResponseError } from "./errors";
import type { Security } from "./models/entities/security";
import type { ParamMetadata } from "./models/requests";
import type { HttpClient, SDKConfiguration } from "./sdkConfiguration";
import type {
	OptionalNullable,
	RetryConfig,
...
import { isUnset, UNSET } from "./types";
...
export class BaseSDK {
	 ...
	 */
	protected getSecurityParams(security: Security | (() => Security) | null): {
	...
	protected async doRequest(
		...
		const execute = async (): Promise<Response> => {
			...
			try {
				...
				let req = request;
				if (hooks) {
					req = await hooks.beforeRequest(
						hookCtx as BeforeRequestContext,
						request,
					);
				}
			...
			if (matchStatusCodes(errorStatusCodes, response.status)) {
				if (hooks) {
					...
					if (result.response) {
						...
						throw new GriddyNFLDefaultError(
		...
		if (
			this.sdkConfiguration.hooks &&
			!matchStatusCodes(errorStatusCodes, response.status)
		) {
	...
	protected async handleJsonResponse<T>(
		...
		const clientErrors = errorStatusCodes.filter((code) =>
			code.startsWith("4"),
		);
		...
		const serverErrors = errorStatusCodes.filter((code) =>
			code.startsWith("5"),
		);
